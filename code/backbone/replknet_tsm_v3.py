...
#     def __init__(self, net, num_segments=3, shift_div=8, drop_path=0.5):
#         super().__init__()
#         self.net = net
...
#         """
#         x1 = self.shift(x, self.num_segments, shift_div=self.shift_div)
#         x1 = self.net(x1)
...
class ConvFFN(nn.Module):
    ...
    def __init__(self, in_channels, internal_channels, out_channels, drop_path, num_segments=6, shift_div=8):
        super().__init__()
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x):
        x1 = shift(x, self.num_segments, self.shift_div)
        out = self.preffn_bn(x1)
        out = self.pw1(out)
        out = self.nonlinear(out)
        out = self.pw2(out)
        return x1 + self.drop_path(out)


class RepLKBlock(nn.Module):

    def __init__(self, in_channels, dw_channels, block_lk_size, small_kernel, drop_path, small_kernel_merged=False, num_segments=6, shift_div=8):
        super().__init__()
...
# 以下定义temporal shift 方法


    # def make_temporal_shift(self):
    #     def make_block_temporal(blocks, num_segments):
    ...
    #         """
    #         blocks = list(blocks.children())
    #         for i, b in enumerate(blocks):
    #             if isinstance(b, RepLKBlock):
    #                 blocks[i] = TemporalShift(
    #                     b, num_segments=num_segments, shift_div=self.shift_div)
    #         return nn.Sequential(*blocks)

    #     for m in self.modules():
    #         if isinstance(m, RepLKNetStage):
    #             m.blocks = make_block_temporal(m.blocks, self.num_segments)
