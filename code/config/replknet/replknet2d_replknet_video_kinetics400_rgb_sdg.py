...
model = dict(
    ...
    backbone=dict(
        ...
        #pretrained2d=True,
        pretrained='./checkpoints/RepLKNet-31B_ImageNet-1K_224.pth',#预训练文件位置
...
# 优化器
optimizer = dict( type='SGD', lr=0.003, momentum=0.9, weight_decay=0.0001)  # 从 0.01 改为 0.005

optimizer_config = dict(grad_clip=dict(max_norm=40, norm_type=2))
# 学习策略
lr_config = dict(policy='step', step=[40, 80]) # step 与 total_epoch 相适应
total_epochs = 100 # 从 100 改为 50 50-》60
checkpoint_config = dict(interval=5)
log_config = dict(interval=20, hooks=[dict(type='TextLoggerHook')])
log_level = 'INFO'

# runtime settings
work_dir = './work_dirs/replknet2d_relplknet_video_kinetics400_rgb_sdg/'